...
namespace Sklad
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (CheckManuf(tbManf.Text.ToString()) && CheckPrice(tbPrice.Text.ToString()))
            {
                if (n.IsCorrect(n.Name) == true && m.IsCorrect(m.Manuf) == true && p.IsCorrect(p.Price) == true && q.IsCorrect(q.Quantity) == true && st.IsCorrect(st.Stocknm) == true)
                {
                ...
                {
                    MessageBox.Show("Something has gone wrong.Check price list");
                }
            }
        }
            ...
            if (temp == true)
            {
                ...
                if (yesno == DialogResult.Yes)
                {
                    ...
                    for (int i = 0; i < Products.Count; i++)
                    {
                        if (Products[i].Stocknm == tbStNum.Text)
                        {
                            temp1 = Products[i].Name + "\t" + Products[i].Quantity + "\n";
                            listBox1.Items.Add(temp1);
//                            listBox1.Items.Add(Products[Products.Count - 1].ToString() + '\n');
                            // listBox1.Text += Products[i].Name + "\t" + Products[i].Quantity + "\n";
                            //  MessageBox.Show(Products[i].Name + "\t" + Products[i].Quantity + "\n");
                        }
                        /*(else
                        {
                            MessageBox.Show("There is no products on the entered stock.");
